  ...
  <div className="grid lg:grid-cols-5 grid-cols-1 gap-10 justify-items-center  ">
    {movieRecom.slice(0,5).map((movie, index) => (
       <Link to={`/movie-details/${movie.id}`}><div key={index} className='flex flex-col text-white gap-2'  onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <img  src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title}  className="h-[300px] z-50 hover:scale-110 mt-5 transition-transform duration-300 w-[200px] object-cover rounded-lg shadow-md"/>
