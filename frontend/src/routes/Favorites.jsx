...
function Favorites() {
  ...
  const filteredFavorites = userFavorites.filter(item => item.mediaType === contentType);
  ...
  const deleteFromList = async (item, type) => {
    try {
      ...
      setUserFavorites(res2.data.user.favorites);
    } catch (error) {
      ...
      <div className='flex flex-col justify-center items-center mt-10'>
        ...
        {loading ? (
          <div className='grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2 justify-center items-center lg:gap-6 gap-2 mx-2 mb-10'>
            <Skeleton MOVIES_PER_PAGE={10} /> {/* Show 10 skeletons */}
          </div>
        ) : filteredFavorites.length === 0 ? (
          <p className="text-gray-400 col-span-5 text-center">
            No favorite {contentType === "movie" ? "movies" : "series"} found.
          </p>
        ) : (
          <div className='grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2 justify-center items-center lg:gap-6 gap-2 mx-2 mb-10'>
            {filteredFavorites.slice().reverse().map((favorite, index) => (
              <Link key={index} to={`/${favorite.mediaType}-details/${favorite.id}`}>
                <div className='flex flex-col text-white gap-2 items-center relative' onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                  <img
                    ...
                    className="lg:h-[300px] z-50 hover:scale-110 mt-5 transition-transform duration-300 w-[200px] object-cover rounded-lg shadow-md"
                  />
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    className='absolute z-50 right-3 bg-red-600 rounded-full p-2 top-7'
                  >
                    <Trash />
                  </button>
                  <p className='text-white px-1 font-bold lg:w-[180px] md:w-[180px] w-[110px] truncate'>
